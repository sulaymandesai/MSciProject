...
class FlatFile:
    ...
    def openFlatFile(self):
        ...
        for i in range(axisCount):
            ...
            if DEBUG:
                print("Axis %i has %i table set(s)".format(i, tableSetCount))
            ...
            for j in range(tableSetCount):
                ...
                if DEBUG:
                    print(
                        "Trigger axis %s has %i intervals.".format(
                            triggerAxisName, intervalCount
                        )
                    )
                ...
                for k in range(intervalCount):
                    ...
                    self.axis[axisName]["tableSets"][triggerAxisName][k][
                        "start"
                    ] = self._readInt()
        ...
        if "TFF_Linear1D" == transferFunctionName:
            transferFunction = (
                lambda z: (z - parameters["Offset"]) / parameters["Factor"]
            )
        elif "TFF_MultiLinear1D" == transferFunctionName:
            transferFunction = (
                lambda z: (parameters["Raw_1"] - parameters["PreOffset"])
                * (z - parameters["Offset"])
                / parameters["NeutralFactor"]
                / parameters["PreFactor"]
            )
        else:
            raise UnhandledTransferFunction(
                "File transfer function: %s is unknown.".format(transferFunctionName)
            )
        ...
        self.creationInformation["date"] = datetime.datetime.fromtimestamp(
            float(self.creationInformation["timestamp"])
        ).isoformat(" ")
        ...
        if self.experimentInfo["Result File Creator"] in self.axis_keys.keys():
            self.axis_keys = self.axis_keys[self.experimentInfo["Result File Creator"]]
        else:
            print(
                "WARNING: Missing axis key for %s,".format(
            ...
            for j in range(parameterCount):
                ...
                if parameterTypeCode == 1:  # 32bits integer
                    ...
                    raise ParameterTypeError(
                        "Unknown parameter type %i given".format(parameterTypeCode)
                    )
        ...
        assert (
            self.file.read() == b""
        ), "There are still some unknown information at the end of the file %s ".format(
            self.filename
        )
    ...
    def _readString(self):
        ...
        if length:
            return str(
                self.file.read(2 * length), encoding="utf16", errors="replace"
    ...
    def _reshapeData(self):
        ...
        info = {
            ...
            % (self.experimentInfo["Run Cycle"], self.experimentInfo["Scan Cycle"]),
            "current": self.experimentElement["Regulator"]["Setpoint_1"]["value"],
            "vgap": self.experimentElement["GapVoltageControl"]["Voltage"]["value"],
            "offset": self.offset,
        }
        ...
        if self.isTopography():
            ...
            sizeX = int(
                self.axis[self.axis_keys["X"]]["clockCount"]
                / (self.axis[self.axis_keys["X"]]["mirrored"] + 1)
            )
            sizeY = int(
                self.axis[self.axis_keys["Y"]]["clockCount"]
                / (self.axis[self.axis_keys["Y"]]["mirrored"] + 1)
            )
            ...
            info.update(
                {
                    ...
                    "xreal": self.axis[self.axis_keys["X"]]["incrementPhysical"]
                    * sizeX
                    * 1e9,
                    "yreal": self.axis[self.axis_keys["Y"]]["incrementPhysical"]
                    * sizeY
                    * 1e9,
                    "unitxy": "nm",
                }
            ...
            self.rawData.resize(
                sizeY * (self.axis[self.axis_keys["Y"]]["mirrored"] + 1),
                sizeX * (self.axis[self.axis_keys["X"]]["mirrored"] + 1),
            )
            ...
            if (
                self.axis[self.axis_keys["X"]]["mirrored"]
                and self.axis[self.axis_keys["Y"]]["mirrored"]
            ):
                info["direction"] = "up-fwd"
                self.data.append(DataArray(self.rawData[0:sizeY, 0:sizeX], info))
                info["direction"] = "up-bwd"
                self.data.append(
                    DataArray(self.rawData[0:sizeY, : sizeX - 1 : -1], info)
                )
                info["direction"] = "down-fwd"
                self.data.append(
                    DataArray(self.rawData[: sizeY - 1 : -1, 0:sizeX], info)
                )
                info["direction"] = "down-bwd"
                self.data.append(
                    DataArray(self.rawData[: sizeY - 1 : -1, : sizeX - 1 : -1], info)
                )
            ...
            sizeV = int(
                self.axis[self.axis_keys["V"]]["clockCount"]
                / (self.axis[self.axis_keys["V"]]["mirrored"] + 1)
            )
            ...
            info.update(
                {
                    ...
                    "vreal": sizeV
                    * self.axis[self.axis_keys["V"]]["incrementPhysical"],
            ...
            mirroredX = (
                len(self.axis[self.axis_keys["V"]]["tableSets"][self.axis_keys["X"]])
                == 2
            )
            mirroredY = (
                len(self.axis[self.axis_keys["V"]]["tableSets"][self.axis_keys["Y"]])
                == 2
            )

            # If I(V) are measured on bwd and fwd, the axis should be mirrored
            if (mirroredX and not self.axis[self.axis_keys["X"]]["mirrored"]) or (
                mirroredY and not self.axis[self.axis_keys["Y"]]["mirrored"]
            ):
                raise UnhandledDataType(
                    "The file %s has an unknown structure".format(self.filename)
                )
            ...
            dataTemp = np.copy(
                self.rawData
            )  # FIXME copy() .... this has huge memory impact
            dataTemp.resize(
                sizeX * (mirroredX + 1) * sizeY * (mirroredY + 1),
                sizeV * (mirroredV + 1),
            )  # each line is a spect. curve
            dataTemp = np.transpose(dataTemp)  # each column is a spectroscopy curve

            # Cut Matrix in two if data are mirrored
            if mirroredV:
                dataTempMirrored = np.copy(
                    dataTemp[: sizeV - 1 : -1, :]
                )  # Reverse order of mirrored data
                dataTemp = np.copy(dataTemp[:sizeV, :])

            dataTemp = np.resize(
                dataTemp, (sizeV, sizeY * (mirroredY + 1), sizeX * (mirroredX + 1))
            )  # slices,cols,rows : 3D view
            if mirroredV:
                dataTempMirrored.resize(
                    sizeV, sizeY * (mirroredY + 1), sizeX * (mirroredX + 1)
                )  # slices,cols,rows : 3D view
            ...
            if mirroredX and mirroredY:
                if DEBUG:
                    print("X and Y mirrored")
                info["direction"] = "up-fwd"
                self.data.append(DataArray(dataTemp[:, 0:sizeY, 0:sizeX], info))
                info["direction"] = "up-bwd"
                self.data.append(
                    DataArray(dataTemp[:, 0:sizeY, : sizeX - 1 : -1], info)
                )
                info["direction"] = "down-fwd"
                self.data.append(
                    DataArray(dataTemp[:, : sizeY - 1 : -1, 0:sizeX], info)
                )
                info["direction"] = "down-bwd"
                self.data.append(
                    DataArray(dataTemp[:, : sizeY - 1 : -1, : sizeX - 1 : -1], info)
                )
                ...
                if mirroredV:
                    ...
                    self.data.append(
                        DataArray(dataTempMirrored[:, 0:sizeY, 0:sizeX], info)
                    )
                    info["direction"] = "up-bwd mirrored"
                    self.data.append(
                        DataArray(dataTempMirrored[:, 0:sizeY, : sizeX - 1 : -1], info)
                    )
                    info["direction"] = "down-fwd mirrored"
                    self.data.append(
                        DataArray(dataTempMirrored[:, : sizeY - 1 : -1, 0:sizeX], info)
                    )
                    info["direction"] = "down-bwd mirrored"
                    self.data.append(
                        DataArray(
                            dataTempMirrored[:, : sizeY - 1 : -1, : sizeX - 1 : -1],
                            info,
                        )
                    )
            ...
            # 2 images up : fwd and bwd
                ...
                if mirroredV:
                    ...
                    info["direction"] = "up-bwd mirrored"
                    self.data.append(
                        DataArray(dataTempMirrored[:, :, : sizeX - 1 : -1], info)
                    )
            ...
            # 2 images fwd : up and down
                ...
                if mirroredV:
                    ...
                    info["direction"] = "down-fwd mirrored"
                    self.data.append(
                        DataArray(dataTempMirrored[:, : sizeY - 1 : -1, :], info)
                    )

            # Only one image
            else:
                if DEBUG:
                    print("X, Y not mirrored")
                info["direction"] = "up-fwd"
            ...
            if DEBUG:
                print(self.axis)
            raise UnhandledDataType(
                "The data file %s has an unhandled type.", format(self.filename)
            )
    ...
    def isTopography(self):
        ...
        if (
            self.dimension == 2
            and self.axis_keys["X"] in self.axis
            and self.axis_keys["Y"] in self.axis
        ):
            return True
        else:
            return False
    ...
    def isGridSpectroscopy(self):
        ...
        if (
            ...
            and self.axis_keys["X"] in self.axis
            and self.axis_keys["Y"] in self.axis
            and self.axis_keys["V"] in self.axis
        ):
            return True
        else:
            return False
